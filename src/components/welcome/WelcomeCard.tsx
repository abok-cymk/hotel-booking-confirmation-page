...
export function WelcomeCard({ host, room, className }: WelcomeCardProps) {
  return (
    <article
      ...
      className={cx(
        "relative flex flex-col justify-between overflow-hidden rounded-2xl --bg-primary p-7 --text-primary-foreground --shadow-(--shadow-float)",
        className,
      )}
    >
      ...
      <div className="relative">
        ...
        <div className="mt-8 space-y-1">
          ...
          <h2 className="font-serif text-4xl leading-none">{host.name}.</h2>
        </div>

        <p className="mt-5 max-w-sm text-sm leading-relaxed --text-primary-foreground/90 --text-pretty">
          {host.message}
        </p>
      </div>
